...
if (Object.values(S_A11Y).some(Boolean)) {
    applyA11yStyles();
            ...
            console.warn('[GOR Script]', e);
        }
    }

    // Trigger on explicit user interaction or deep idle
    if (document.readyState === 'complete') {
        if ('requestIdleCallback' in window) {
            requestIdleCallback(initApp, { timeout: 4000 });
        } else {
    ...
    // True user intent triggers
    ['pointerdown', 'touchstart', 'keydown', 'click'].forEach(function(evt) {
        window.addEventListener(evt, initApp, { once: true, passive: true });
    });
})();