...
// ==========================================================================
// GOR CRM - UNIVERSAL AUTH & USER PROFILE ENGINE (Desktop & Mobile)
// ==========================================================================
(function initGorCrmAuthEngine() {
    const STORAGE_KEY = 'gor_crm_session_v1';
    ...
    function renderNavbarAuth() {
        ...
        const session = getSession();
        const navContainers = document.querySelectorAll('.nav-links');
        const mobileControls = document.querySelectorAll('.navbar-mobile-controls, .mobile-header-crm-wrap');

        // 1. Render in Desktop Nav
        navContainers.forEach(nav => {
            const existingBtn = nav.querySelector('.btn-crm-nav');
            const existingWidget = nav.querySelector('.crm-user-profile-widget');
            if (existingBtn) existingBtn.remove();
        ...
        mobileTriggers.forEach(mobBtn => {
            if (session && session.isLoggedIn) {
                ...
                mobBtn.innerHTML = '<i class="fas fa-user-shield"></i> <span>CRM</span>';
